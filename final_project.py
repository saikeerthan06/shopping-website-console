...
def show_items_for_sale(shopping_cart):
    ...
    try:
      for choice in item_choices:
        item_number = int(choice.strip())
        if 1 <= item_number <= len(catalog_PG, Catalog_RG, Catalog_MG, Catalog_MG1, Catalog_M):
            selected_products.append(catalog_PG, Catalog_RG, Catalog_MG, Catalog_MG1, Catalog_M[item_number - 1])
        else:
            print(f"Invalid Item Number, please try again")
    except ValueError:
        print("Invalid Input, please try again")
    if selected_products:
        print("You have added: {item_number} to the shopping cart")
...
def checkout(membership_check, generate_OTP):
  ...
  def membership_check():
    ...
    for pdt_names6 in adding_item_to_cart:
...
homepage()
type_handling()
show_items_for_sale(shopping_cart)
